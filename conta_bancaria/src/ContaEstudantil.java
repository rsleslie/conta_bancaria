public class ContaEstudantil extends Conta {
    private double emprestimoEstudantil = 5000;
    ...
    public ContaEstudantil() 
    {
		...
		for (int i = 1; i <= 10; i++) 
		{
			tipoMovimento = visualizar("CONTA ESTUDANTIL");
    ...
    public void usarEmprestimo(double valor) {
        if (valor <= emprestimoEstudantil) {
			emprestimoEstudantil -= valor;
			credito(valor);
